...
const GameCard = ({ game, wishlist,screenBig=false }: { game: Game; wishlist?: boolean,screenBig:boolean }) => {
  ...
  return (
    <HoverCard >
      <div className="mb-4 md:mb-0 flex relative flex-col items-start gap-4">
      ...
      <HoverCardContent align="center" className="w-full bg-transparent border-none">
        {game.short_screenshots && (
          <ImageSwitcher game={game} key={game.id} images={screenBig?game.short_screenshots:game.user_game} />
        )}
      </HoverCardContent>
    </HoverCard>
  );
