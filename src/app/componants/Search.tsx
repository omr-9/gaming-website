...
const Search = () => {
  ...
  const { games, isLoading } = useGetGame({ query: search, isDisabled: search === "" });
  ...
  return (
    ...
    >
      ...
      />
       {query === "" && search=== "" ? <SearchIcon className="w-5 h-5 duration-200 group-hover:text-rose-400" /> : ( <XIcon className=" cursor-pointer" onClick={() => {
        setQuery('')
        setSearch("")
      }} />) }
      <AnimatePresence>
        {(games?.data || isLoading) && active && (
          <MotionItem
            ...
            animate={{ height: "auto" }}
            exit={{ opacity: 0 }}
            className="absolute w-full top-full z-50 bg-[#222425] rounded-2xl shadow-sm max-h-[40vh] hide-scrollbar overflow-y-scroll left-0"
          >
            {isLoading ? (
